...
const STREAK_KEY = "india2world-streak";
...
function updateStreak(): StreakData {
  ...
  if (existing.lastDate === today) return existing; // already counted today

  const yesterday = new Date(Date.now() - 86_400_000).toISOString().slice(0, 10);
  const isConsecutive = existing.lastDate === yesterday;

  const updated: StreakData = {
    count: isConsecutive ? existing.count + 1 : 1,
    lastDate: today,
  };
...
function MiniGauge({ score }: { score: number }) {
  ...
  const strokeColor =
    score >= 75 ? "#138808" : score >= 40 ? "#FF9933" : "#ef4444";

  return (
    <div className="flex flex-col items-center gap-1">
      <svg width="60" height="60" viewBox="0 0 60 60" className="shrink-0 drop-shadow-sm">
        <circle
          cx="30"
          cy="30"
          r={r}
...
function StreakBadge({ count }: { count: number }) {
  ...
  return (
    <div className="flex items-center gap-1 rounded-full bg-orange-100 px-2.5 py-1 ring-1 ring-orange-200 dark:bg-orange-900/30 dark:ring-orange-800/50">
